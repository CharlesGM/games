...
SCREEN_WIDTH = 600
SCREEN_HEIGHT = 950
PLAY_WIDTH = 350  # 10 blocks wide
PLAY_HEIGHT = 840  # 24 blocks high
BLOCK_SIZE = 35
...
def valid_space(shape, grid):
    accepted_pos = [[(j, i) for j in range(len(grid[0])) if grid[i][j] == (0,0,0)] for i in range(len(grid))]
...
def main(win):
    ...
    def reset_game():
        ...
        level_time = 0
        score = 0
        return locked_positions, grid, current_piece, next_piece, change_piece, fall_time, fall_speed, level_time, score

    locked_positions, grid, current_piece, next_piece, change_piece, fall_time, fall_speed, level_time, score = reset_game()
    ...
    while run:
        ...
        if game_over:
            draw_text_middle(win, "YOU LOST!", 80, (255,255,255), y_offset=-50)
            draw_text_middle(win, "Press R to Restart or Q to Quit", 40, (255,255,255), y_offset=20)
